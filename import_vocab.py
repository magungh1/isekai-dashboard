    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row_num, row in enumerate(reader, 1):
            ...
            if kana_type not in ("hiragana", "katakana"):
                print(f"  Line {row_num}: skipped "
                      f"(type must be 'hiragana' or 'katakana', got '{kana_type}')")
                skipped += 1
                continue

            try:
                conn.execute(
                    "INSERT INTO kana_srs (word, meaning, type) VALUES (%s, %s, %s)",
