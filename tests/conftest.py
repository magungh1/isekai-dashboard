...
# ── Guard: Supabase mode skips SQLite fixtures ──────────────────
if os.environ.get("USE_SUPABASE"):
    # In Supabase mode, services connect directly to the cloud DB.
    # Set SUPABASE_URL + SUPABASE_SERVICE_KEY to run tests.
    pass
else:
    ...
    def use_temp_db(tmp_path, monkeypatch):
        ...
        # Override the DB path via monkeypatch on the internal helper
        monkeypatch.setattr(db_module, "_get_db_url", lambda: test_db)

        # Reset any cached pool/connection state
        ...
        # Re-grab a fresh connection and create tables
        conn = db_module.get_shared_connection()
        conn.execute('''
